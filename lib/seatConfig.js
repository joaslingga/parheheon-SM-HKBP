...
// Returns seat IDs split by status: pending (kuning) and approved (putih/abu)
export async function getBookedSeatIdsByStatus() {
  const rows = await db
    .prepare(
      `SELECT sb.seat_id, r.status FROM seat_bookings sb
       JOIN reservations r ON sb.reservation_id = r.id
       WHERE r.status IN ('pending', 'approved')`
    )
    .all();
  const pending = rows.filter((r) => r.status === "pending").map((r) => r.seat_id);
  const approved = rows.filter((r) => r.status === "approved").map((r) => r.seat_id);
