...
async function initializeSchema(pool) {
  ...
  // 6. voting_records
  await request.query(`
    IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'voting_records')
    BEGIN
  ...
  await request.query(`
    ...
    BEGIN
      INSERT INTO users (username, password, role, name, coins) VALUES ('superadmin', 'super123', 'superadmin', 'Super Admin', 0);
    END
    IF NOT EXISTS (SELECT 1 FROM users WHERE username = 'admin')
    BEGIN
      INSERT INTO users (username, password, role, name, coins) VALUES ('admin', 'admin123', 'admin', 'Admin Utama', 0);
    END
    IF NOT EXISTS (SELECT 1 FROM users WHERE username = 'user')
    BEGIN
      INSERT INTO users (username, password, role, name, coins) VALUES ('user', 'user123', 'user', 'Siswa Sekolah Minggu', 0);
    END
  `);

  // Seed default candidates
  const countCandidates = await request.query(`SELECT COUNT(*) as count FROM candidates`);
  if (countCandidates.recordset[0].count === 0) {
    await request.query(`
      INSERT INTO candidates (name, votes_count) VALUES ('Kelas Balita - Koor Pujian Anak', 0);
      INSERT INTO candidates (name, votes_count) VALUES ('Kelas Pratama - Tari Kreasi Sukacita', 0);
      INSERT INTO candidates (name, votes_count) VALUES ('Kelas Remaja - Band Kidung Pujian', 0);
    `);
  }
